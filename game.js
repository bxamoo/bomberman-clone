...
let explosions=[], explosionTimer=0;
let lookX=0, lookY=1;
let enemyCooldown=0;
...
function resetStage(){
    ...
    player={x:1,y:1};
    enemy={x:13,y:13,alive:true,escaping:false};
    bomb=null;
    explosions=[];
    explosionTimer=0;
    enemyCooldown=0;
...
function updateEnemy(){
    ...
    if(bomb){
        ...
        for(const d of DIRS){
            ...
            let safe=true;
            for(const e of getExplosionTiles(bomb)){
                if(e.x===nx&&e.y===ny) safe=false;
            }
            if(safe){
...
function drawEyes(cx,cy,dx,dy){
    ...
    for(const s of[-1,1]){
        ctx.fillStyle="white";
        ctx.beginPath();
        ctx.arc(cx+s*6,cy-4,4,0,Math.PI*2);
        ctx.fill();
        ctx.fillStyle="black";
        ctx.beginPath();
        ctx.arc(cx+s*6+dx*2,cy-4+dy*2,2,0,Math.PI*2);
        ctx.fill();
    }
}
...
function draw(){
    ...
    for(const e of explosions){
        ...
        ctx.beginPath();
        ctx.arc(e.x*TILE+16,e.y*TILE+16,18,0,Math.PI*2);
        ctx.fill();
    }
